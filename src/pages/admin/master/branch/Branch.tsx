...
export default function Branches() {
  ...
  const [formModal, setFormModal] = useState<Branchs | null>(null);
  const [viewId, setViewId] = useState<number | null>(null);

  const loadData = useCallback(() => {
    getBranch({ page, per_page: perPage, search }).then((res) => {
      console.log("response ", res);

      const response = res?.pms_branches;
  ...
  const handleToggle = async (id: number, currentStatus: boolean) => {
    try {
      await updateStatusBranch(id, { active: !currentStatus });
  ...
  const handleSave = async () => {
    try {
      if (formModal?.id && formModal.id !== 0) {
        await updateStatusBranch(formModal.id, { name: formModal.name });
  ...
  const columns: {
    header: string;
    accessor: keyof Branchs;
    render?: (user: Branchs, index: number) => React.ReactNode;
  }[] = [
    {
      header: "Sr No.",
      accessor: "id",
      render: (_row: Branchs, index: number) =>
  ...
  return (
    <>
      ...
      {formModal && (
        <Modal
          title={formModal.id ? "Edit Branch" : "Create Branch"}
      ...
      {/* View Modal */}
      {viewId && (
        <Modal title="Branch Details" onClose={() => setViewId(null)}>
