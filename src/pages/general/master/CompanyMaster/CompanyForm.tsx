...
const CompanyForm: React.FC<CompanyFormProps> = ({
      ...
      if (
        ...
        const gstinEntries = data.company_gstin_details.map(
          (
            ...
            pms_state_id: item.pms_state_id 
              ? (typeof item.pms_state_id === 'string' ? parseInt(item.pms_state_id) : item.pms_state_id)
              : 0,
            _destroy: false,
          })
        );
        ...
        gstinEntries.forEach((entry, index) => {
          ...
          setValue(
            ...
            typeof entry.pms_state_id === 'string' ? parseInt(entry.pms_state_id) || 0 : entry.pms_state_id || 0
          );
        });
      } else {
        setGstinEntries([]);
  ...
  const orgOptions = mapToOptions(
    dropDown?.locations?.organizations || []
  ...
  const countryOptions = mapToOptions(
    dropDown?.locations?.countries || []
  ) as Array<{ value: number; label: string }>;
  const allStateOptions = mapToOptions(
    dropDown?.locations?.states || []
  ) as Array<{ value: number; label: string }>;
  ...
  const addGSTINEntry = () => {
    ...
    setTimeout(() => {
      console.log(`Setting form values for new GSTIN entry at index ${newIndex}`);
      ...
      console.log(`Form values set for GSTIN entry ${newIndex}`);
    }, 100);
  };
      ...
      const updatedEntries = prevEntries
        .map((entry) => {
          if (entry.id === idToRemove) {
            // Mark existing entries for destruction
            if (!entry.id.startsWith("temp_")) {
              console.log(`Marking GSTIN entry ${entry.id} for destruction`);
              return { ...entry, _destroy: true };
            }
            // Remove temporary entries completely
            console.log(`Removing temporary GSTIN entry ${entry.id}`);
            return null;
          }
          return entry;
        })
        .filter(Boolean) as GSTINEntry[];

      console.log("Updated GSTIN entries:", updatedEntries);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Configurations */}
        <CompanyConfigurations register={register} isReadOnly={isReadOnly} />
        ...
        <CompanyLogoUpload 
          isReadOnly={isReadOnly}
          setValue={setValue}
          existingLogo={companyData?.company_logo ? {
            id: companyData?.company_logo?.id,
            url: companyData?.company_logo?.document,
            filename: companyData?.company_logo?.filename || "company-logo",
            active: true,
          } : undefined}
        />
