...
import { getFloor, updateFloor } from "../../../../services/General/floorServices";

export default function FloorList() {
  const navigate = useNavigate();
  const [states, setStates] = useState<Floor[]>([]);
  ...
  const loadData = useCallback(() => {
    getFloor({ page, per_page: perPage, search }).then((res) => {
      setStates(res.pms_floors || []);
  ...
  const handleToggle = async (userId: number, currentStatus: boolean) => {
    ...
    try {
      await updateFloor(userId, { active: newStatus });
  ...
  const handelDelete = async (id: number) => {
    ...
    try {
      await updateFloor(id, { deleted: true });
  ...
  const handleEdit = (state: Floor) => {
    navigate(`${state.id}/edit`);
  };

  const columns: {
    header: string;
    accessor: keyof Floor;
    render?: (Floor: Floor, index: number) => React.ReactNode;
  }[] = [
    {
      header: "Sr No.",
      accessor: "id",
