...
export async function getCountry(params: FetchCountryParams) : Promise<PaginatedCountryResponse> {
    const {search, ...rest} = params;
    const rasackQuery = search ? {'q[country_code_or_country_or_region_or_isd_code_cont]': search} : {};
...
export async function createCountry(userData: Country) {
    return axiosInstance.post('/pms/countries', userData);
}
...
// Update User
export async function updateStatusCountry(id: number, data: Partial<Country>): Promise<Country> {
  const response = await axiosInstance.patch(`/pms/countries/${id}.json`, data);
  return response.data;
}
